...
export class Mesh {
    ...
    cube(width: number = 1, height: number = 1, length: number = 1, widthSegments: number = 1, heightSegments: number = 1, lengthSegments: number = 1) {
        ...
        function getVertexIndex(xi: number, yi: number, zi: number): number {
            return zi + (lengthSegments + 1) * (yi + (heightSegments + 1) * xi);
        }
